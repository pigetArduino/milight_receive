...
void setup()
{
  ...
  printf_begin();
  delay(1000);
  Serial.println("Press a button on your smartphone to receive radiocodes");
  mlr.begin();
}
        ...
        switch (inChar) {
          ...
          case '.':
            if (state == COMPLETE) {
              ...
              Serial.println(outgoingPacket[0]);
              Serial.println(outgoingPacket[1]);
              Serial.println(outgoingPacket[2]);
              Serial.println(outgoingPacket[3]);
              Serial.println(outgoingPacket[4]);
              Serial.println(outgoingPacket[5]);
              Serial.println(outgoingPacket[6]);
            }
            if(inChar != ' ') {
              outgoingPacketPos = 0;
              state = IDLE;
